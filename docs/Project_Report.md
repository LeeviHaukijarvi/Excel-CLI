...
```
          ...
          ▼                       ▼                       ▼
┌─────────────────┐   ┌─────────────────┐   ┌─────────────────────┐
│   TextContent   │   │ NumericContent  │   │   FormulaContent    │
│─────────────────│   │─────────────────│   │─────────────────────│
│ - text: String  │   │ - number: double│   │ - rawFormula: String│
│─────────────────│   │─────────────────│   │ - expression: Expr  │
│ + getValue()    │   │ + getValue()    │   │─────────────────────│
│ + getRawContent()│  │ + getRawContent()│  │ + getValue()        │
└─────────────────┘   └─────────────────┘   │ + getRawContent()   │
                                            │ + getExpression()   │
                                            └─────────────────────┘
```
...
```
        ...
        ▼               ▼           ▼           ▼               ▼
┌───────────┐   ┌─────────────┐ ┌───────┐ ┌──────────┐  ┌────────────────┐
│  Literal  │   │CellReference│ │ Range │ │ Function │  │BinaryOperation │
│───────────│   │─────────────│ │───────│ │──────────│  │────────────────│
│-value:dbl │   │-coordinate  │ │-start │ │-name:Str │  │-left: Expr     │
│───────────│   │-spreadsheet │ │-end   │ │-argument │  │-right: Expr    │
│+calculate()│  │─────────────│ │-sheet │ │──────────│  │-operator: char │
└───────────┘   │+calculate() │ │───────│ │+calculate│  │────────────────│
                │+getCoord()  │ │+calc()│ │+getArg() │  │+calculate()    │
                └─────────────┘ │+getVal│ └──────────┘  │+getLeft()      │
                                │+getAllCoords()│       │+getRight()     │
                                └───────┘               └────────────────┘
```
...
│ + reset(): void                                                      │
│ - collectDependents(cell: String, result: Set<String>): void        │
│ - dependsOn(cell: String, target: String): boolean                  │
...
│                        FileSystem                                    │
│─────────────────────────────────────────────────────────────────────│
│ + save(sheet: Spreadsheet, path: String): void                      │
│ + load(path: String): Spreadsheet                                   │
│ + columnNumber(coord: String): int                                  │
│ + rowNumber(coord: String): int                                     │
│ + coordinate(col: int, row: int): String                            │
└─────────────────────────────────────────────────────────────────────┘
```
...
For generating visual class diagrams, use the following PlantUML code:

### Full Class Diagram (PlantUML)

```plantuml
@startuml Excel-CLI Class Diagram
